...
m = mujoco.MjModel.from_xml_path('1_snake_forward_ft_final.xml')
d = mujoco.MjData(m)
...
video_name = "1_snake_forward_ft_final.mp4"
...
for _ in range(render_length):  # Number of frames per simulation step
    ...
    if count % 2 == 0:
        d.ctrl[0] = desired_angle_normalized[idx][0] * 1.49 # Apply normalized angles
        d.ctrl[1] = desired_angle_normalized[idx][1] * 1.49
        d.ctrl[2] = desired_angle_normalized[idx][2] * 1.8
        d.ctrl[3] = desired_angle_normalized[idx][1] * 1.94 #1.94 for forward
        d.ctrl[4] = desired_angle_normalized[idx][0] * 1.94 #1.94 for forward
...
# Define colors
colors = plt.cm.viridis(np.linspace(0, 1, m.nu))  # Unique colors for each joint
...
colors = np.array([
    ...
    [0,0,0,1],
    [1,1,1,1],
],dtype=np.float32)# Unique colors for each joint
print(colors,colors.shape)
...
# for i in range(m.nu):
#     plt.grid(True)
#     # Sensor values (lighter color)
#     plt.plot(time_stamps, joint_values[:, i], color=colors[i], linewidth=1.5, alpha=0.5, label=f'Sensor {i+1}')
#     # Actuator values (solid color)
#     plt.plot(time_stamps, actuation_values[:, i], color=colors[i], linewidth=2, linestyle='--', label=f'Actuator {i+1}')
#     # MSE values (dashed black)
#     plt.plot(time_stamps, mse_values[:, i], color=colors[i], linewidth=1, alpha=0.6, label=f'MSE {i+1}' if i == 0 else "")

# for i in range(m.nu):
#     # plt.subplot(m.nu, 2*i + 1)  # Left column: Sensor & Actuation
#     plt.plot(time_stamps, actuation_values[:, i], color=colors[i], linewidth=0.5, label=f'Actuator {i+1}')  # Solid color
#     plt.plot(time_stamps, joint_values[:, i], color=colors[i], linewidth=0.5, alpha=0.5, label=f'Sensor {i+1}')  # Lighter color
#     plt.ylabel(f'Joint {i+1}')
#     plt.legend()
#     plt.grid(True)
#     # plt.subplot(m.nu, 2, 2*i + 2)  # Right column: MSE
#     # plt.plot(time_stamps, mse_values[:, i], color=colors[i], linewidth=0.5)
#     # plt.ylabel(f'MSE {i+1}')
#     # plt.grid(True)
#     # Plot results
...
for i in range(m.nu):
    ...
    plt.plot(time_stamps[:250], actuation_values[:250, i], label=f'$q_{i+1}$ desired', linestyle='--', linewidth=1.5, color='r')
    plt.plot(time_stamps[:250], joint_values[:250, i], label=f'$q_{i+1}$ actual', linewidth=1.5, color='b')
    plt.ylabel(f'Joint : ($q_{i+1}$)')
    plt.grid(True)
