...
# time points: based on dt and t_span
...
normalize_min = -135
normalize_max = 135
desired_angle_normalized = (
    desired_angle - normalize_min) / (normalize_max - normalize_min) * 2 - 1
...
for i in range(n):
    ...
    plt.plot(t[start_idx:end_idx],
             desired_angle[start_idx:end_idx, i], linewidth=1.5)
    plt.ylabel(f'x_{i+1}')
    plt.grid(True)
    # plt.ylim(-1, 1)
    plt.ylim(-90, 90)
...
m = mujoco.MjModel.from_xml_path('1_snake_rotate_ft.xml')
d = mujoco.MjData(m)
...
video_name = "1_snake_rotate_ft.mp4"
...
if cam_id == -1:
    print(f"Warning: Camera '{
          camera_name}' not found in XML. Using default camera.")
else:
    print(f"Using predefined camera: {camera_name}")
...
def compute_com(data):
    ...
    com = np.sum(data.xipos[body_indices] *
                 body_mass[:, None], axis=0) / total_mass
    return com


# Get body IDs for two end-effectors
site_id_ee1 = m.body("snake").id  # First end-effector
...
for _ in range(render_length):  # Number of frames per simulation step
    ...
    if count % 2 == 0:
        d.ctrl[0] = desired_angle_normalized[idx][0] * \
            1.49  # Apply normalized angles
        d.ctrl[1] = desired_angle_normalized[idx][1] * 1.49
        d.ctrl[2] = desired_angle_normalized[idx][2] * 1.8
        d.ctrl[3] = desired_angle_normalized[idx][1] * - \
            1.49  # 1.94 for forward
        d.ctrl[4] = desired_angle_normalized[idx][0] * - \
            1.49  # 1.94 for forward
    ...
    jd_end_eff_1.append(ee1_pos)
    jd_end_eff_2.append(ee2_pos)
    jd_COM.append(com_pos)
    # Append to trajectory lists
    traj_ee1.append(ee1_pos)
    traj_ee2.append(ee2_pos)
    traj_com.append(com_pos)
    joint_values.append(d.sensordata.copy())  # Store joint positions
    # Store actuation values
    actuation_values.append(desired_angle_normalized[idx].copy())
    ...
    rgba_ee1 = np.array([1.0, 0.0, 0.0, 1.0], dtype=np.float32).reshape(
        4, 1)  # Red for EE1
    rgba_ee2 = np.array([1.0, 0.0, 0.0, 1.0], dtype=np.float32).reshape(
        4, 1)  # Green for EE2
    rgba_com = np.array([0.0, 0.0, 1.0, 1.0], dtype=np.float32).reshape(
        4, 1)  # Blue for COM
    ...
    for i in range(1, len(traj_ee1)):
        if renderer._scene.ngeom < renderer._scene.maxgeom:
            ...
            mujoco.mjv_initGeom(
                geom, mujoco.mjtGeom.mjGEOM_LINE, size, pnt1, mat, rgba_ee1)
            mujoco.mjv_connector(
                geom, mujoco.mjtGeom.mjGEOM_LINE, 2, pnt1, pnt2)
            renderer._scene.ngeom += 1
            ...
            if i < len(traj_ee2):
                ...
                pnt1_ee2 = np.array(
                    traj_ee2[i - 1], dtype=np.float64).reshape(3, 1)
                pnt2_ee2 = np.array(
                    traj_ee2[i], dtype=np.float64).reshape(3, 1)
                mujoco.mjv_initGeom(
                    geom_ee2, mujoco.mjtGeom.mjGEOM_LINE, size, pnt1_ee2, mat, rgba_ee2)
                mujoco.mjv_connector(
                    geom_ee2, mujoco.mjtGeom.mjGEOM_LINE, 2, pnt1_ee2, pnt2_ee2)
                renderer._scene.ngeom += 1
            ...
            if i < len(traj_com):
                ...
                pnt1_com = np.array(
                    traj_com[i - 1], dtype=np.float64).reshape(3, 1)
                pnt2_com = np.array(
                    traj_com[i], dtype=np.float64).reshape(3, 1)
                mujoco.mjv_initGeom(
                    geom_com, mujoco.mjtGeom.mjGEOM_LINE, size_com, pnt1_com, mat, rgba_com)
                mujoco.mjv_connector(
                    geom_com, mujoco.mjtGeom.mjGEOM_LINE, 2, pnt1_com, pnt2_com)
                renderer._scene.ngeom += 1
...
jd_end_eff_1 = np.array(jd_end_eff_1)
jd_end_eff_2 = np.array(jd_end_eff_2)
jd_COM = np.array(jd_COM)
...
plt.plot(time_stamps[:500], mse_values[:500],
         label="MSE", linewidth=1.5, color='b')
plt.xlabel('Time (s)')
plt.legend()
plt.grid(True)
plt.show()
...
# Close video writer
scipy.io.savemat(video_name + 'sensor_actuator_data.mat', {
    'joint_values': joint_values,  # Sensor data
    'actuation_values': actuation_values,  # Actuator data
    'time_stamps': time_stamps  # Time vector
})

scipy.io.savemat(video_name + '_x_y_data.mat', {
    'jd_end_eff_1': jd_end_eff_1,
    'jd_end_eff_2': jd_end_eff_2,
    'jd_COM': jd_COM,
