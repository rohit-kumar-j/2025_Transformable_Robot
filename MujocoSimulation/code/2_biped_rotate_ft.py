...
R_ampLF = np.array([30, -80,  45, 0,  0.0])
R_ampRF = np.array([30, -80, -45, 0, -0.0])
...
OffsetsLF = np.array([0, 0, 0, -90, -90])
OffsetsRF = np.array([0, 60, -0, -90, 90])
...
m = mujoco.MjModel.from_xml_path('2_biped_rotate_ft.xml')
d = mujoco.MjData(m)
...
video_name = "2_biped_rotate_ft.mp4"
...
site_id_ee1 = m.body("m6_r").id  # First end-effector
site_id_ee2 = m.body("m6_l").id  # Second end-effector
...
for _ in range(render_length):  # Number of frames per simulation step
    ...
    if count % 3 == 0:
        ...
        d.ctrl[5:10] = desired_angle_rf_normalized[idx]*0.1
    ...
    idx += 1
    count += 1
    if idx >= desired_angle_lf_normalized.shape[0]:
        idx = 0
        print("reset\n")
