...
m = mujoco.MjModel.from_xml_path('3_dual_snake_rotate_ft.xml')
d = mujoco.MjData(m)
...
video_name = "3_dual_snake_rotate_ft.mp4"
...
for _ in range(render_length):  # Number of frames per simulation step
    ...
    if count % 2 == 0:
        ...
        d.ctrl[3] = desired_angle_normalized[idx][1] * -1.49 #1.94 for forward
        d.ctrl[4] = desired_angle_normalized[idx][0] * -1.49 #1.94 for forward
        ...
        d.ctrl[8] = -desired_angle_normalized[idx][1] * -1.49 #1.94 for forward
        d.ctrl[9] = -desired_angle_normalized[idx][0] * -1.49 #1.94 for forward
